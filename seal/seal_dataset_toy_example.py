...
def extract_enclosing_subgraphs(df, link_index, edge_index, num_hops, y):
    ...
    data_list = []
    draw_graph(edge_index)
    for src, dst in link_index.t().tolist(): # The times of loop execution = #links = #samples
        src_origin = src
        dst_origin = dst
        sub_nodes, sub_edge_index, mapping, _ = k_hop_subgraph(
            [src, dst], num_hops, edge_index,
            relabel_nodes=False)  # relabel Ture, label index in sub_edge_index will be changed.
        draw_graph(sub_edge_index)
        sub_nodes, sub_edge_index, mapping, _ = k_hop_subgraph(
            [src, dst], num_hops, edge_index, relabel_nodes=True) # relabel Ture, label index in sub_edge_index will be changed.
...
def drnl_node_labeling(edge_index, src, dst, num_nodes=None):
    ...
    idx = list(range(src)) + list(range(src + 1, adj.shape[0])) # get the id list without src node
    adj_wo_src = adj[idx, :][:, idx] # get the adjacency matrix without src node

    idx = list(range(dst)) + list(range(dst + 1, adj.shape[0]))
    adj_wo_dst = adj[idx, :][:, idx]

    dist2src = shortest_path(adj_wo_dst, directed=False, unweighted=True)  # shortest path for between nodes
    dist2src = dist2src[:, src] # Get the distance from each node to node src+1
    dist2src = np.insert(dist2src, dst, 0, axis=0) # insert array of 0 before dst
    dist2src = torch.from_numpy(dist2src)

    dist2dst = shortest_path(adj_wo_src, directed=False, unweighted=True)
    dist2dst = dist2dst[:, dst - 1] # Get the distance from each node to node dst-1
